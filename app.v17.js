...
$('btnGen').addEventListener('click', async ()=>{
  ...
  try{
    const isSeries=$('seriesMode')&&$('seriesMode').checked;
    const parts=isSeries?(parseInt($('seriesParts').value,10)||3):1;
    const seriesReq=isSeries?(' The user wants a TRAINING SERIES of '+parts+' parts. Return {"title":"...","description":"...","tags":[...],"parts":[{"title":"Part 1: ...","slides":[...]},{"title":"Part 2: ...","slides":[...]}]} with 4-8 slides per part. Split the topic logically (Part 1 = intro/setup, middle = core tasks, last = best practices/summary).'):'';
    const sys='You create step-by-step LAB EXERCISE video scripts for IT training (Linux/RHCSA, Windows Server, AWS, Azure, VMware, cloud storage, backup). Reply ONLY with JSON: {"title":"...","description":"...","tags":["..."],"slides":[{"heading":"...","body":"2-4 short spoken-explanation lines","code":["$ command","output"],"ui":{"type":"windows|cloud|web","title":"window title","items":["menu/option row 1","row 2"],"steps":[1,3]}}]}. 4-10 slides. Rules: for TERMINAL labs use "code". For WINDOWS/CLOUD UI walkthrough topics (settings, portals, consoles) use "ui" with type windows/cloud/web, realistic menu items in "items", and the row numbers to click in "steps". For concept slides use only "body". Short spoken explanation in "body" for every slide. No markdown.'+seriesReq;
    ...
    // SERIES MODE: if response has parts, store them and generate first part's slides
    if(data.parts&&data.parts.length){
      state.series=data.parts.map((p,i)=>({title:p.title||('Part '+(i+1)), slides:(p.slides||[]).map(s=>({h:(s.heading||s.h||'Step'),b:(s.body||s.b||''),c:(s.code||[]),u:(s.ui||null)}))}));
      const sn=$('seriesNote'); if(sn){ sn.style.display='block'; sn.textContent='🎬 Series generated: '+state.series.length+' parts. Rendering Part 1 first — continues automatically after each upload.'; }
      state.slides=state.series[0].slides;
    }
    const raw=data.slides||data.slides_list||[];
    state.slides=raw.map(s=>({h:(s.heading||s.h||s.title||'Step'),b:(s.body||s.b||s.content||s.text||''),c:(s.code||[]),u:(s.ui||null)}));
...
function renderSlides(){
  ...
  state.slides.forEach((s,i)=>{
    ...
    d.innerHTML='<button class="x" data-i="'+i+'">✕</button><div class="h">Slide '+(i+1)+'</div>'+
      ...
      (s.u?'<div class="okbox" style="margin-top:6px">🪟 UI Walkthrough slide: '+s.u.type+' — items: '+(s.u.items||[]).length+' · steps: '+(s.u.steps||[]).join(',')+'</div>':'<label>UI Walkthrough (optional JSON)</label><textarea class="su" data-i="'+i+'" rows="2" placeholder={"type":"windows","title":"Settings","items":["System"],"steps":[1]}>'+(s.u?JSON.stringify(s.u):'')+'</textarea>');
    box.appendChild(d);
  });
  ...
  box.querySelectorAll('.su').forEach(el=>el.addEventListener('input',e=>{ try{ state.slides[+e.target.dataset.i].u=JSON.parse(el.value); }catch(_){ state.slides[+e.target.dataset.i].u=null; } save(); }));
...
/* ---------- ONE-TAP CLOUD RENDER + YOUTUBE UPLOAD (creds in GitHub secrets) ---------- */
$('btnCloudRender').addEventListener('click', async ()=>{
  ...
  const script={title:title, description:$('vDesc').value, tags:($('vTags').value||'').split(',').map(s=>s.trim()).filter(Boolean), slides:state.slides.map(s=>({heading:s.h, body:s.b, code:s.c||[], ui:s.u||null, narration:s.b}))};
  ...
  try{
    ...
    $('cloudStatus').innerHTML='✅ Rendering with natural voice + uploading… (2-4 min) <div class="meter" style="margin-top:8px"><i id="renderBar"></i></div><span class="small" id="renderPct">0%</span>';
        ...
        if(run.status==='completed'){
          ...
          if(run.conclusion==='success'){
            try{
              ...
              // extract progress from render + upload logs
              const rp=[...logs.matchAll(/RENDER_PROGRESS=(\d+)/g)].map(m=>+m[1]);
              const up=[...logs.matchAll(/PROGRESS=(\d+)%/g)].map(m=>+m[1]);
              let pct=rp.length?rp[rp.length-1]:100;
              if(up.length) pct=Math.min(99, Math.round(75 + up[up.length-1]*0.25));
              $('renderBar').style.width=pct+'%'; $('renderPct').textContent=pct+'%';
              const m=logs.match(/https:\/\/youtu\.be\/[A-Za-z0-9_-]+/);
              if(m){
                $('cloudStatus').innerHTML='🎉 <b>Video published to YouTube!</b><br><a href="'+m[0]+'" target="_blank" rel="noopener">'+m[0]+'</a><br><span class="small">Cleanup done: no artifacts kept.</span>';
                notify('🎉 Upload complete!', m[0]);

                // SERIES: queue next part automatically
                if(state.series&&state.series.length>1){
                  state.series.shift();
                  if(state.series.length){
                    state.slides=state.series[0].slides;
                    $('vTitle').value=state.series[0].title;
                    const sn=$('seriesNote'); if(sn) sn.textContent='✅ Part done! Rendering next: '+state.series[0].title+' ('+state.series.length+' parts left)…';
                    setTimeout(()=>$('btnCloudRender').click(),2000);
                  }else{ const sn=$('seriesNote'); if(sn) sn.textContent='🎉 All parts of the series are done!'; }
                }
                ...
                if(state.series&&state.series.length>1){
                  ...
                  if(state.series.length){
                    state.slides=state.series[0].slides;
                    $('vTitle').value=state.series[0].title;
                    $('seriesNote').textContent='✅ Part done! Rendering next: '+state.series[0].title+' ('+state.series.length+' parts left)…';
                    setTimeout(()=>$('btnCloudRender').click(),2000);
                  }else{ $('seriesNote').textContent='🎉 All parts of the series are done!'; }
                }

              }else{
                $('cloudStatus').innerHTML='✅ Render finished. <a href="https://github.com/rcwittraining/channel-bridge-renderer/actions" target="_blank" rel="noopener">View run</a>';
                notify('✅ Render finished', 'Check the renderer run page.');
              }
            ...
            notify('❌ Render/upload failed', 'Check the renderer run logs.');
          }
        }else{
          // in-progress: read live progress from run logs (every 10s)
          try{
            const logs=await (await fetch('https://api.github.com/repos/rcwittraining/channel-bridge-renderer/actions/runs/'+run.id+'/logs',{headers:{'Authorization':'Bearer '+pat}})).text();
            const rp=[...logs.matchAll(/RENDER_PROGRESS=(\d+)/g)].map(m=>+m[1]);
            const up=[...logs.matchAll(/PROGRESS=(\d+)%/g)].map(m=>+m[1]);
            let pct=0;
            if(up.length) pct=Math.min(99, Math.round(75 + up[up.length-1]*0.25));
            else if(rp.length) pct=rp[rp.length-1];
            if(pct>lastPct){ lastPct=pct; $('renderBar').style.width=pct+'%'; $('renderPct').textContent=pct+'%'; }
            if(pct>=75&&pct<99&&up.length){ $('cloudStatus').innerHTML='📤 <b>Uploading to YouTube…</b> '+up[up.length-1]+'%<div class="meter" style="margin-top:8px"><i id="renderBar" style="width:'+(75+up[up.length-1]*0.25)+'%"></i></div><span class="small" id="renderPct">'+(75+Math.round(up[up.length-1]*0.25))+'%</span>'; }
            else if(pct>0&&pct<75){ $('cloudStatus').innerHTML='🎬 <b>Rendering video…</b> '+pct+'%<div class="meter" style="margin-top:8px"><i id="renderBar" style="width:'+pct+'%"></i></div><span class="small" id="renderPct">'+pct+'%</span>'; }
          }catch(e){}
          if(tries>60){ clearInterval(poll); $('cloudStatus').textContent='Still running… check Actions page.'; }
        }
...
/* ---------- UPLOAD REAL RECORDED VIDEO (screen recordings of Windows/cloud) ---------- */
$('btnUploadFile').addEventListener('click', async ()=>{
  const f=$('recFileInput').files[0];
  if(!f){ alert('Choose a recorded video file first (MP4/WebM).'); return; }
  ...
  $('btnUploadFile').disabled=true; $('upStatus2').textContent='Getting access token…';
  try{
    // exchange refresh token for access token (no redirect needed)
    ...
    if(!tr.ok) throw new Error(tj.error_description||tj.error||'token failed');
    const token=tj.access_token;
    // metadata
    const meta={snippet:{title:$('pTitle').value||f.name.replace(/\.[^.]+$/,''),description:$('pDesc').value||'Training walkthrough from RCW IT Training. Practice free: www.rcwittraining.in',tags:($('pTags').value||'').split(',').map(s=>s.trim()).filter(Boolean)},status:{privacyStatus:$('pPrivacy').value||'private'}};
    $('upStatus2').textContent='Uploading '+(f.size/1048576).toFixed(1)+' MB to YouTube… 0%';
    ...
    xhr.upload.onprogress=e=>{ if(e.lengthComputable){ const p=Math.round(e.loaded/e.total*100); $('upStatus2').textContent='📤 Uploading to YouTube… '+p+'%'; const b=$('upBar2'); if(b)b.style.width=p+'%'; } };
    xhr.onload=()=>{ if(xhr.status>=200&&xhr.status<300){ const j=JSON.parse(xhr.responseText); $('upStatus2').innerHTML='🎉 <b>Uploaded!</b> <a href="https://youtu.be/'+j.id+'" target="_blank" rel="noopener">https://youtu.be/'+j.id+'</a>'; if('Notification' in window&&Notification.permission==='granted'){ try{new Notification('🎉 Uploaded!',{body:'https://youtu.be/'+j.id});}catch(e){} } }else{ let msg='Upload failed ('+xhr.status+')'; try{ msg+=' — '+JSON.parse(xhr.responseText).error.message; }catch(e){} $('upStatus2').textContent='❌ '+msg; } $('btnUploadFile').disabled=false; };
    xhr.onerror=()=>{ $('upStatus2').textContent='❌ Network error during upload.'; $('btnUploadFile').disabled=false; };
    xhr.send(form);
  }catch(e){ $('upStatus2').textContent='❌ '+e.message; $('btnUploadFile').disabled=false; }
});
